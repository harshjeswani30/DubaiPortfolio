...
function useParallax(value: MotionValue<number>, distance: number) {
  ...
  color1: "rgba(234, 179, 8, 0.08)",
  color2: "rgba(0, 0, 0, 0.05)",
  angle: 135,
}

const hoverGradient = {
  color1: "rgba(234, 179, 8, 0.18)",
...
export function HeroSection() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative h-[120vh] overflow-hidden bg-background text-foreground"
    >
      <motion.div 
        ...
        <motion.div
          ...
          animate={{
            background: isCardHovered 
              ? "radial-gradient(ellipse at 70% 50%, rgba(234, 179, 8, 0.08) 0%, transparent 50%)"
              : "radial-gradient(ellipse at 70% 50%, rgba(234, 179, 8, 0) 0%, transparent 50%)",
          }}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
          ...
          >
            <div className="absolute inset-0 rounded-full border border-primary-medium/20" />
            <div className="absolute inset-[100px] rounded-full border border-accent-light/10" />
            <div className="absolute inset-[200px] rounded-full border border-foreground/5" />
          </motion.div>
          
          <motion.div
            style={{ x: smoothX, y: smoothY }}
            className="absolute left-1/4 top-1/3 h-[400px] w-[400px] rounded-full bg-primary-medium/20 blur-[100px]"
            animate={{
              backgroundColor: isCardHovered ? "rgba(234, 179, 8, 0.15)" : "rgba(0, 0, 0, 0.1)",
            }}
            transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          />
          <motion.div
            ...
            animate={{
              backgroundColor: isCardHovered ? "rgba(0, 0, 0, 0.05)" : "rgba(234, 179, 8, 0.05)",
            }}
            transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}
          />
          ...
          <motion.div
            ...
            className="absolute left-10 top-1/4 h-2 w-2 rounded-full bg-accent-light"
          />
          <motion.div
            style={{ y: floatingY1 }}
            className="absolute right-1/4 top-1/3 h-3 w-3 rounded-full bg-primary-medium"
          />
          <motion.div
            style={{ y: floatingY2 }}
            className="absolute left-1/3 bottom-1/4 h-1.5 w-1.5 rounded-full bg-foreground/50"
          />
        </div>
        ...
        >
          ...
          >
              <motion.div
                ...
                className="mb-4"
              >
                <div className="group relative inline-flex items-center gap-3 rounded-full border border-primary-medium/30 bg-primary-medium/5 px-5 py-2.5 overflow-hidden cursor-pointer hover:border-accent-light/50 transition-colors">
                  <motion.div
                    className="absolute inset-0 bg-gradient-to-r from-accent-light/0 via-accent-light/10 to-accent-light/0"
                    animate={{ x: ["-100%", "100%"] }}
                  ...
                  <div className="relative flex items-center">
                    <RotatingText
                      ...
                      mainClassName="overflow-hidden justify-center text-sm font-medium text-foreground"
                      staggerFrom="last"
                      initial={{ y: "100%" }}
                      animate={{ y: 0 }}
                      exit={{ y: "-120%" }}
              ...
              >
                ...
                <span className="relative mx-2 inline-block">
                  ...
                  <motion.span
                    ...
                    className="absolute -bottom-1 left-0 h-1 rounded-full bg-gradient-to-r from-primary-dark to-accent-light"
                  />
                </span>
                <br />
                Experiences
              ...
              <motion.p
                ...
                className="mt-4 max-w-md text-sm text-primary-dark/70 md:text-base leading-relaxed"
              >
              ...
              >
                <Link href="/projects">
                  <motion.button
                    ...
                    className="group relative flex items-center gap-2 overflow-hidden rounded-xl bg-primary-dark px-5 py-3 text-sm font-semibold text-background transition-all"
                  >
                    <motion.div
                ...
                <Link href="/contact">
                  <motion.button
                    whileHover={{ scale: 1.02, y: -2 }}
                    whileTap={{ scale: 0.98 }}
                    className="rounded-xl border-2 border-primary-medium/20 bg-transparent px-5 py-3 text-sm font-semibold text-foreground transition-all hover:border-accent-light hover:bg-accent-light/5"
                  >
                    Get in Touch
                  </motion.button>
                </Link>
                ...
                {stats.map((stat, i) => (
                  ...
                  >
                    <motion.div
                      ...
                      className="flex items-center gap-2 rounded-xl border border-primary-medium/20 bg-primary-medium/5 px-3 py-2.5 backdrop-blur-sm transition-all hover:border-accent-light/50 hover:bg-primary-medium/10"
                    >
                      <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary-medium/10">
                        <stat.icon className="h-4 w-4 text-accent-light" />
                      </div>
                      <div>
                        <div className="text-lg font-bold text-foreground">{stat.value}</div>
                        <div className="text-[10px] text-primary-dark/60 whitespace-nowrap">{stat.label}</div>
                      </div>
                    </motion.div>
                  </motion.div>
                ))}
              ...
              {isCardHovered && (
                ...
                >
                    <motion.div
                      className="absolute inset-0 rounded-full"
                      style={{
                        background: "radial-gradient(circle, rgba(234, 179, 8, 0.2) 0%, rgba(234, 179, 8, 0.08) 30%, transparent 60%)",
                        filter: "blur(30px)",
                      }}
                      animate={{
                        scale: [1, 1.05, 1],
          ...
          >
            ...
            >
              <span className="text-[10px] text-accent-light/60">Scroll</span>
              <div className="h-8 w-4 rounded-full border border-primary-medium/30 p-0.5">
                <motion.div
                  animate={{ y: [0, 12, 0] }}
                  transition={{ duration: 2, repeat: Infinity }}
