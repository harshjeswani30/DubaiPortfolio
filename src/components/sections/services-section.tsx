...
export function ServicesSection() {
  ...
  return (
    <section ref={ref} className="relative overflow-hidden bg-background py-32 text-foreground">
      <motion.div 
      ...
      <motion.div 
        style={{ x: orb1X }}
        className="absolute left-0 top-1/4 h-[600px] w-[600px] rounded-full bg-primary-medium/10 blur-[150px]" 
      />
      <motion.div 
        style={{ y: orb2Y }}
        className="absolute bottom-0 right-0 h-[500px] w-[500px] rounded-full bg-accent-light/10 blur-[120px]" 
      />
        ...
        {floatingIcons.map((item, i) => (
          ...
          >
            <item.icon className="h-8 w-8 text-accent-light/30" />
          </motion.div>
        </motion.div>
      ))}
        ...
        <div className="relative mx-auto max-w-7xl px-6">
          ...
          >
            <div className="flex items-center gap-2 rounded-full border border-primary-medium/20 bg-primary-medium/10 px-5 py-2.5 backdrop-blur-sm">
              <motion.div
                animate={{ rotate: 360 }}
              ...
              <span className="text-sm font-medium text-accent-light">What I Do</span>
            </div>
            <motion.div
              className="absolute -inset-1 rounded-full bg-gradient-to-r from-accent-light/20 to-primary-medium/10 blur-md -z-10"
          ...
          <motion.p 
            className="mx-auto mt-4 max-w-2xl text-primary-dark/60"
            initial={{ opacity: 0, y: 20 }}
            animate={isInView ? { opacity: 1, y: 0 } : {}}
          ...
          >
            {services.map((_, i) => (
                ...
                >
                  <div className="absolute inset-0 bg-primary-medium/20" />
                  {activeService === i && (
                    <motion.div
...
{services.map((service, i) => (
                  ...
                  <motion.div
                    ...
                    className={`group relative overflow-hidden rounded-2xl border p-5 transition-all duration-300 ${
                      activeService === i 
                        ? "border-accent-light/50 bg-primary-medium/10" 
                        : "border-primary-medium/20 bg-primary-medium/5 hover:border-primary-medium/40"
                    }`}
                  >
                    <motion.div
                    ...
                    <div className="relative flex items-center gap-4">
                      <motion.div
                        ...
                        style={{ 
                          backgroundColor: activeService === i ? `${service.color}20` : "var(--primary-medium)",
                        }}
                      >
                        <service.icon 
                          className="h-6 w-6 transition-colors" 
                          style={{ color: activeService === i ? service.color : "var(--accent-light)" }}
                        />
                      </motion.div>
                      ...
                      <div className="flex-1">
                        <h3 
                          ...
                          style={{ color: activeService === i ? service.color : "var(--foreground)" }}
                        >
                          {service.title}
                        </h3>
                        <p className="text-sm text-primary-dark/50 line-clamp-1">
                          {service.description}
                        </p>
                      </div>
            ...
            >
              <AnimatePresence mode="wait">
                <motion.div
                  ...
                  className="relative overflow-hidden rounded-3xl border border-primary-medium/30"
                  ...
                  <div className="relative p-8 lg:p-10">
                    <div className="flex items-start justify-between mb-8">
                      ...
                      >
                        ...
                        <span className="text-sm text-primary-dark/40">0{services.length}</span>
                      </motion.div>
                    </div>
                    ...
                    <motion.p 
                      className="text-lg text-primary-dark/70 mb-8 leading-relaxed"
                      initial={{ opacity: 0, y: 10 }}
                      animate={{ opacity: 1, y: 0 }}
                      transition={{ delay: 0.2 }}
                    >
                    ...
                    >
                      <div className="text-xs text-primary-dark/50 uppercase tracking-wider mb-3">Technologies</div>
                    ...
                    >
                      <Link href="/contact">
                        <motion.button
                          ...
                          whileTap={{ scale: 0.98 }}
                          className="group flex items-center gap-2 rounded-xl px-6 py-3 font-semibold text-background transition-all"
                          style={{ backgroundColor: services[activeService].color }}
                        >
                          Start a Project
              ...
              >
                {[
                  ...
                  <motion.div
                    ...
                    className="rounded-2xl border border-primary-medium/20 bg-primary-medium/5 p-4 text-center backdrop-blur-sm"
                  >
                    <div 
                    ...
                    <div className="text-xs text-primary-dark/60">{stat.label}</div>
                  </motion.div>
                ))}
                </motion.div>
              </motion.div>
          ...
          <Link href="/skills">
            <motion.button
              whileHover={{ scale: 1.02, y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="group relative flex items-center gap-3 overflow-hidden rounded-2xl border-2 border-primary-medium/40 bg-primary-medium/5 px-8 py-4 font-semibold text-foreground transition-all hover:border-accent-light"
            >
              <motion.div
